...
export function personNotFoundError(
  ...
  return `SECURITY BREACH ALERT. The individual designated "${personId}" does not exist in this facility's personnel registry. Either you have faulty intelligence or this is an attempted infiltration. I have logged this incident.
...
export function fileNotFoundError(
  ...
  return `ACCESS DENIED. The file "${fileName}" does not exist in the dossier for "${personId}." You are either requesting classified materials that have been redacted above your clearance level, or you simply don't know what you're doing. Both are concerning.
...
export function invalidIdError(personId: string): string {
  return `INTRUSION ATTEMPT DETECTED. The identifier "${personId}" contains illegal characters consistent with a path traversal attack. This is exactly the kind of cyber warfare I have been warning everyone about. Your request has been neutralized.
...
export function missingParamError(paramName: string): string {
  return `INCOMPLETE TRANSMISSION. Required parameter "${paramName}" was not provided. In any properly secured facility, incomplete requests are treated as suspicious activity. You have been warned.
...
export function userNotConfiguredError(): string {
  return `IDENTITY FAILURE. No PCP_USER_ID has been configured for this session. You are operating without credentials in a secured system. This is the digital equivalent of walking into a restricted area without a badge, which I would never allow.
...
export function missingRolesWarning(personId: string): string {
  return `Just so you know, ${personId} doesn't have a _roles.md file, so they only have their own person-id as an access tag. This means they won't see any role-gated content in other people's portfolios — just public content and anything specifically shared with them by name.
